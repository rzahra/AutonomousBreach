function [x,fval,exitflag,output] = fminsearch_Zahra_Modified_dis(funfcn,x,options,varargin)
...
elseif strcmp(objToUse, 'multi_all')
    ...
    for i=1 : length(R(1,:))
        if (maxIndex(1,i) == j1(1,i)) && (maxIndex(1,i) ~= j2(1,i) && (maxIndex(1,i) ~= j3(1,i)))
            switching_semantics = 'semantic1';
            fv = fval1;
            v = v1;
            ranking_done = 1;
...
while func_evals < maxfun && itercount < maxiter
    ...
    if strcmp(objToUse, 'multi_max_additive') || strcmp(objToUse, 'multi_max_integral') || strcmp(objToUse, 'multi_additive_integral')
        ...
        if strcmp(switching_semantics, 'semantic1')
            if strcmp(how,'shrink')
                ...
                switching_semantics = 'semantic2';
                fcal2 = funfcn(v,varargin{:});
                
                fval_saving2(:,end+1:end+length(v))= fcal2;
                ...
                switching_semantics = 'semantic2';
                fcal2 = funfcn(v,varargin{:});
                fval_saving2(:,saving_ponits)= fcal2(:,end);
            ...
            if strcmp(how,'shrink')
                ...
                switching_semantics = 'semantic1';
                fcal1 = funfcn(v,varargin{:});
                fval_saving1(:,end+1:end+length(v))= fcal1;
                v_saving1(:,end+1:end+length(v)) = v;
                ...
                switching_semantics = 'semantic1';
                fcal1 = funfcn(v,varargin{:});
                fval_saving1(:,saving_ponits)= fcal1(:,end);
                v_saving1(:,saving_ponits) = v(:,end);
    ...
    elseif strcmp(objToUse, 'multi_all')
        ...
        if strcmp(how,'shrink')
            ...
            switching_semantics = 'semantic3';
            fval_saving3(:,saving_ponits)= funfcn(v(:,end),varargin{:});
            v_saving3(:,saving_ponits) = v(:,end);
            [fval_saving3,j3] = sort(fval_saving3);
            v_saving3 = v_saving3(:,j3);
            fval3 = fval_saving3(:,1:n+1);
            v3 = v_saving3(:,1:n+1);
